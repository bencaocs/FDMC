function words_son =random_Decode_fun(base_son,txt,anssor,k_max)%函数功能，给与任意长度的ATGC序列，转为对应单词，txt是和anssor对应的碱基子序列
            ...
            for g=1:size(txt(1:size(anssor,1)),1)
                if strcmp(base_son(k_min_min:k),char(txt(g,:)))==1%说明现在还不是最后一个后面还有
                    if k+1<=size(base_son,2)&&strcmp(base_son(k+1),'G')==1
                        ...
                        decode_num=1;
                        base_son=base_son(k+2:size(base_son,2));
                        random_Decode_fun(base_son,txt,anssor,size(base_son,2)-k);
                        words_son=out_word;
                        return
                    end
                end
